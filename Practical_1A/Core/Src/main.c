...
void change_timer_period(uint32_t new_period_ms)
{
	...
	// Timer clock = 1000 Hz (Prescaler = 7999), so 1 tick = 1 ms.
	uint32_t new_arr = 0; // Replace 0 with your calculation.
	new_arr = new_period_ms - 1;
...
void handle_buttons(void)
{
    ...
    uint8_t pressed0 = (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_RESET);
    uint8_t pressed1 = (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_1) == GPIO_PIN_RESET);
    uint8_t pressed2 = (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_2) == GPIO_PIN_RESET);
    uint8_t pressed3 = (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_3) == GPIO_PIN_RESET);
    ...
    if ((now - last_button_time[0] > DEBOUNCE_MS) && pressed0 && !last_state0)
    {
    ...
    last_state0 = pressed0;

    /* TODO: Implement debounce logic for PA1. Call set_mode(MODE_1). */
    if ((now - last_button_time[1] > DEBOUNCE_MS) && pressed1 && !last_state1)
    {
        last_button_time[1] = now;
        set_mode(MODE_1);
    }
    last_state1 = pressed1;

    /* TODO: Implement debounce logic for PA2. Call set_mode(MODE_2). */
    if ((now - last_button_time[2] > DEBOUNCE_MS) && pressed2 && !last_state2)
    {
        last_button_time[2] = now;
        set_mode(MODE_2);
    }
    last_state2 = pressed2;

    /* TODO: Implement debounce logic for PA3. Call set_mode(MODE_3). */
    if ((now - last_button_time[3] > DEBOUNCE_MS) && pressed3 && !last_state3)
    {
        last_button_time[3] = now;
        set_mode(MODE_3);
    }
    last_state3 = pressed3;
}
...
void mode3_update(void)
{
    ...
    switch (sparkle_state) {
        case SPARKLE_IDLE:
            ...
            sparkle_display_until = now + (100 + (rand() % 1401));
        ...
        case SPARKLE_DISPLAY:
            ...
            if (now >= sparkle_display_until)
            {
                ...
                {
                    sparkle_next_off_time = now + (100 + (rand() % 51)); // 100-150ms
